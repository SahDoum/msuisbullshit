...
// @downloadURL  https://github.com/askesis/bot/raw/master/botForMSU.user.js
// @updateURL    https://github.com/askesis/bot/raw/master/botForMSU.user.js
// @grant        none
// ==/UserScript==
...
function MyPixelBot() {
  ...
  MyPixelBot.url = {
    ...
    image: 'https://raw.githubusercontent.com/SahDoum/msuisbullshit/master/masked_msu_bordered.png',
  };
  ...
  MyPixelBot.canvasClick = function(x, y, color) {
    ...
    var pxColor = MyPixelBot.getColor(
      MyPixelBot.ctx.getImageData(x, y + 1, 1, 1).data,
      0
    );
    var colorEl = document.querySelectorAll(
    ...
    if (!colorEl) {
      ...
      if (MyPixelBot.doCoordLog) {
        console.log(
          '== ' + x + offset + 'x' + y + '%c ' + pxColor,
          'background:' + pxColor + ';'
        );
        MyPixelBot.setState(
          'Пропускаю ' + (x + offset + 1) + 'x' + (y + 1) + ' совпал цвет'
        );
      } else {
        console.log('==');
      ...
      if (MyPixelBot.doCoordLog) {
        console.log(
          x + offset + 'x' + y + '%c ' + pxColor + ' -> %c ' + color,
          'background:' + pxColor + ';',
          'background:' + color + ';'
        );
        MyPixelBot.setState(
          'Поставил точку ' + (x + offset + 1) + 'x' + (y + 1)
        );
      } else {
        console.log(' -> ');
  ...
  MyPixelBot.getFullData = function() {
    MyPixelBot.pixs = [];
    MyPixelBot.pixs = MyPixelBot.randomShuffle(
      MyPixelBot.getData(0).concat(MyPixelBot.getData(795))
    );
  ...
  MyPixelBot.getData = function(offsetX) {
    ...
    for (var i = 0; i < id1.length; i += 4) {
      var x = offsetX + (i / 4) % 795,
        y = ~~(i / 4 / 795);
      if (
        MyPixelBot.getColor(id1, i) !== MyPixelBot.getColor(id2, i) &&
        MyPixelBot.getColor(id2, i) !== MyPixelBot.tc
      ) {
        data.push([
          x,
          y,
          MyPixelBot.getColor(id2, i),
          MyPixelBot.getColor(id1, i),
        ]);
      }
    }
    return data;
  ...
  MyPixelBot.wait = setInterval(function() {
    ...
    if (window.localStorage.getItem('DROP_FIRST_TIME') != '1') {
      ...
      MyPixelBot.canvas &&
      document.querySelectorAll('.Ttl > .Ttl__wait').length
    ) {
      MyPixelBot.timer = 1;
